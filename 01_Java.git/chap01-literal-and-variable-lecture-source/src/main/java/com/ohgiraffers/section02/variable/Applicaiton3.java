...
//        int for = 20;       //예약어 사용 불가
        ...
        int Age = 20;         //위에서 만든 age와 다른 것으로 취급한다. (대문자)
        int True = 10;        //예약어 true와 다른 것으로 취급한다. (대문자 True는 예약어가 아님)
...
//        int sh@rp = 20;     //사용 가능한 특수문자 외에는 사용 불가능
        ...
        int slajkefsdjfaieowfkldlfkajkelfsdkfnawvmdsfklaflkmfwlkwef; // 에러는 안나지만 너무 김

        int maxAge = 20;  //카멜케이스(camel-case) <-앞 소문자, 뒤 대문자로 표현하는 방식
        int minAge = 10;

        /* 단어와 단어 사이의 연결을 언더스코어(_)로 하지 않는다. */
        String user_name; //이렇게 사용 X
        String userName;  //올바른 표현

        int 나이;          //가능하지만 권장하지 않음

        String s;         //변수가 어떤 의미인지 파악하기 힘들다.
        String name;      //문자열 형태의 이름이 저장되겠구나 하는 의도가 파악됨
