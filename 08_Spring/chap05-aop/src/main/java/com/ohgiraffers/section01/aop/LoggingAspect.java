...
public class LoggingAspect {
    ...
    //*(..) : 매개변수가 있든 없든, 몇 개든 상관 없다는 표현식
    public void LogPointCut() {}

    /* JoinPoint : 포인트컷으로 패치한 실행 시점
    * 이렇게 매치된 조인포인트에서 해야 할 일이 어드바이스이다.
    * 매개변수로 전달한 JoinPoint 객체는 현재 조인 포인트의 메소드명, 인수값등의 자세한 정보를 엑세스 할 수 있다.
    * */
    ...
    * 단, 패키지가 다르다면 패키지를 포함한 클래스명을 기술해야 한다.
    * */
    @After("LogPointCut()")
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        /* 원본 조인포인트를 실행한다. 빼면 실행 X */
        Object result = joinPoint.proceed();    // 실행할 지 결정
