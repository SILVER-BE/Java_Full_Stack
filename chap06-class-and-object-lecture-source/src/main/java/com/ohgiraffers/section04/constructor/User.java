...
public class User {
    ...
    *       this.필드명 = 매개변수;    //설정자(setter) 여러 개의 기능을 한 번의 호출로 수행 가능
    * }
    *
    * [ 생성자 작성 시 주의할 점 ]
    * 1. 생성자 메소드는 반드시 클래스 이름과 동일해야 한다. (대/소문자까지 같아야 함.)
...
//        this.enrollDate = enrollDate;
        ...
        * this()는 가장 첫 줄에 선언해야 하며, 그렇지 않은 경우 Compile Error가 발생한다. */
        ...
        this(id, pwd, name); // 미리 작성한 세개의 필드를 초기화하는 생성자로 매개변수로 받은 값을 전달
                             // (위에 작성한 this.id, this.pwd, this.name 을 호출해서 쓰는 것)
        this.enrollDate = enrollDate; // 위에 작성하지 않았기 때문에 작성하여 초기화
