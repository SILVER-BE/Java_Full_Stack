...
public class NativeQueryRepository {
    ...
    public Menu nativeQueryResultType(int menuCode) {
        /* Native Query 수행 결과를 엔티티에 매핑 시키려면 모든 컬럼이 조회되어야만 매핑이 가능하다.  */
        String query = "SELECT menu_code, menu_name, menu_price, category_code, orderable_status "
                + "FROM tbl_menu WHERE menu_code = ?";
        //SELECT문 하나라도 빠지면 오류남(엔티티를 출력해야하기 때문에, 엔티티에 있는 모든 속성을 조회)
    ...
    public List<Object[]> nativeQueryByBoResultType() {
