...
public class SimpleJPQLRepository {
    ...
    public String selectSingleMenuByTypedQuery() {  // FROM 절에는 Entity 명
        String jpql = "SELECT m.menuName FROM Section01Menu as m WHERE m.menuCode = 8";
        TypedQuery<String> query = entityManager.createQuery(jpql, String.class); // 수행하고 나서 결과로 받을 타입(String 타입으로 반환을 받겠다.)
    ...
    public Object selectSingleMenuByQuery() {
        String jpql = "SELECT m.menuName FROM Section01Menu as m WHERE m.menuCode = 8";
        Query query = entityManager.createQuery(jpql); // 수행하고 나서 결과로 받을 타입(String 타입으로 반환을 받겠다.)
        Object resultMenuName = query.getSingleResult();

        return resultMenuName;
    }

    public Menu selectSingleRowByTypedQuery() {  // FROM 절에는 Entity 명
        String jpql = "SELECT m FROM Section01Menu as m WHERE m.menuCode = 8";
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class); // 수행하고 나서 결과로 받을 타입(String 타입으로 반환을 받겠다.)
    ...
    public List<Integer> selectUsingDistinct() {
        String jpql = "SELECT DISTINCT m.categoryCode FROM Section01Menu as m"; // 중복 값을 제외
